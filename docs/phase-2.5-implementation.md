...
`ReasonFact` records fact kind, key, label, value, unit, stable entity/event sources, and capture tick. Utility factors may carry one of these fact snapshots, and decision records retain a deterministic strongest reason. Implementation-only continuity/tie-break factors are excluded from strongest-reason selection.
...
| --------------------- | --------------: | --------------------------------------------------------------------------------------------------------------------------------- |
| Simulation behavior   |               2 | Same seed/commands are comparable only within behavior v2. Intent and slot changes intentionally broke v1 hash equivalence.       |
| Authoritative state   |               2 | State v1 migrates deterministically to the v2 intent, route, claim, reason, and attention shape.                                  |
| Command envelope      |               1 | Shape remains v1 and carries behavior v2. Scheduled commands inside legacy replays/experiments are preserved by those migrations. |
...
| Scenario reference    |               1 | Current references carry behavior v2. Legacy embedded experiment scenarios are upgraded with the experiment.                      |
...
| Outcome               |               1 | Regenerated from current state and tagged with behavior v2; no persisted legacy migration is exposed.                             |
...
Schema version and behavior version answer different questions. Replay, save, scenario, and outcome envelopes remain schema 1 while declaring behavior 2. Experiment schema 2 adds `responseTrace`; the nested trace starts at schema 1 and does not change replay schema because it records observational evidence rather than simulation input.

## Migration and rejection behavior

| Input                                                                                         | Implemented result                                                                                                                                                                                                                                                                          |
| --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| State v1                                                                                      | Clone input; add bounds, routes, and intent history; enrich decisions and events; derive active desire/plan; rebuild valid claims where possible; validate as state v2.                                                                                                                     |
| Save schema 1 with behavior/state v1                                                          | Run the state migration, validate the candidate, and return a save schema 1 envelope tagged behavior/state v2. The runtime parses and hashes the candidate before replacing its active state.                                                                                               |
| Replay schema 1 with behavior/state v1                                                        | Preserve seed and ordered command log; emit behavior/state v2; remove legacy `finalTick` and `finalHash`, so the replay is unverified until run under v2.                                                                                                                                   |
| Experiment schema 1 with behavior/state v2                                                    | Do not mutate the input; preserve branches, commands, outcomes, labels, bookmarks, target/hash pairs, and checkpoints; emit schema 2 with `responseTrace: null` on every command-log entry.                                                                                                 |
...
Migration is intentionally asymmetric. Current v2 artifacts are validated and preserved; v1 artifacts may retain historical commands and labels, but no v1 hash is silently reinterpreted as v2 proof.

...
from observations that have actually been collected. A command or template is
not counted as a passing result.
...
| Current repository verification                     | Formatting, lint, type-checking, 438 tests, all three coverage suites, golden verification, build, and bundle budgets pass. The complete Chromium suite passes 44/44 with 34 visual baselines.                                                                                                                                            | **Pass**                     |
| Chromium/Firefox/WebKit release smoke               | The final release matrix passes 12/12 in 2.6 minutes: 4/4 in Chromium, 4/4 in Firefox, and 4/4 in WebKit.                                                                                                                                                                                                                                 | **Pass**                     |
...
The final local Windows run on 5 August 2026 passed 12/12 in 2.6 minutes, with
all four tagged journeys passing in each browser. Routine `npm run test:e2e`
...
The formative and confirmatory rounds must use different groups of five people.
Record task accuracy, time, route, missed cues, vocabulary confusion, false
causal claims, and accessibility/input failures. Resolve blocker and
high-severity patterns before the confirmatory round. At least four of the five
confirmatory participants must satisfy every core comprehension criterion.
